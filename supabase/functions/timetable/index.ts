...
Deno.serve(async (req) => {
  ...
  try {
    if (method === 'GET') {
      ...
      const day = getSearchParams(req).get('day');
      let q = supabase.from('timetable').select('*, subjects(name, code)').order('day_of_week').order('start_time');
      if (!isAdm) q = q.eq('teacher_id', userId);
      if (className) q = q.eq('class', className);
      if (day !== null) q = q.eq('day_of_week', parseInt(day));
      const { data } = await q;
    ...
    if (method === 'POST') {
      ...
      const { data, error } = await supabase.from('timetable').insert({ teacher_id: userId, ...body }).select().single();
    ...
    if (method === 'PUT' && id) {
      const body = await req.json();
      let q = supabase.from('timetable').update(body).eq('id', id);
      if (!isAdm) q = q.eq('teacher_id', userId);
      const { data, error } = await q.select().single();
      if (error || !data) return json({ success: false, error: 'Entry not found' }, 404);
      return json({ success: true, data });
    }

    if (method === 'DELETE' && id) {
      let q = supabase.from('timetable').delete().eq('id', id);
      if (!isAdm) q = q.eq('teacher_id', userId);
      await q;
